...
package com.cxgps.vehicle.widget.wheel;
...
public class WheelView extends View {
	...
	private void initResourcesIfNecessary() {
		...
		setBackgroundResource(R.drawable.date_wheel_bg);
	}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			double textWidth = Math.floor(Layout.getDesiredWidth("0",
					itemsPaint));
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int)  Math.floor(Layout.getDesiredWidth(label,
					valuePaint));
		}
