package lfeng.com.imdeveloper.ui.view.citypicker;
...
public class ScrollerNumberPicker extends View {
	...
	/** 选中的时候字体 */
	private float selectedFont = 18.0f;
	/** 单元格高度 */
	...
	private void drawMask(Canvas canvas) {
		...
		LinearGradient lg2 = new LinearGradient(0, controlHeight - maskHight, 0,
				controlHeight, 0x00f2f2f2, 0x00f2f2f2, TileMode.MIRROR);
		Paint paint2 = new Paint();
		paint2.setShader(lg2);
		canvas.drawRect(0, controlHeight - maskHight, controlWidth, controlHeight,
				paint2);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		unitHeight = (int) attribute.getDimension(R.styleable.NumberPicker_unitHight,
				32);
		normalFont = attribute.getDimension(R.styleable.NumberPicker_normalTextSize,
				14.0f);
		selectedFont = attribute.getDimension(
				R.styleable.NumberPicker_selecredTextSize, 18.0f);
		itemNumber = attribute.getInt(R.styleable.NumberPicker_itemNumber, 7);
		normalColor = attribute.getColor(R.styleable.NumberPicker_normalTextColor,
				0xff000000);
		selectedColor = attribute.getColor(R.styleable.NumberPicker_selecredTextColor,
				0xffff0000);
	...
	private class ItemObject {
		...
		public void drawSelf(Canvas canvas) {
			...
			// 绘制内容
			canvas.drawText(itemText, x + controlWidth / 2 - textRect.width() / 2, y
					+ move + unitHeight / 2 + textRect.height() / 2, textPaint);
		...
		public boolean isSelected() {
			...
			if ((y + move + unitHeight) >= controlHeight / 2 - unitHeight / 2 + 2
					&& (y + move + unitHeight) <= controlHeight / 2 + unitHeight / 2
							- 2)
				return true;
			if ((y + move) <= controlHeight / 2 - unitHeight / 2 + 2
					&& (y + move + unitHeight) >= controlHeight / 2 + unitHeight / 2
