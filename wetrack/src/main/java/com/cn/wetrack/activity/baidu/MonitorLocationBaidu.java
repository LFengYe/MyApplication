    ...
    private void addMarkersToMap() {
        ...
        if (CarMarker == null) {
            CarMarker = (Marker) mBaiDuMap.addOverlay(new MarkerOptions()
                    .position(point)
                    .icon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("", 0)))
                    .title("marker")
                    .anchor(0.5f, 0.5f)
                    .extraInfo(bundle));
            ...
            CarMarker.setIcon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("", 0)));
            ...
            if (isonline(serverCalendar, calendar)) {
                if (null == location.getAlarmmsg() || location.getAlarmmsg().length() <= 0) {
                    if (location.getVelocity() < 2) {
                        ...
                        CarMarker.setIcon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("", location.getAngle())));
                    }
                } else {
