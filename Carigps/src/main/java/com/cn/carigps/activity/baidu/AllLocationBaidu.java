...
import com.cn.carigps.activity.Dizhijiexi;
import com.cn.carigps.activity.ExpiredListAdapter;
import com.cn.carigps.activity.MyApplication;
import com.cn.carigps.activity.SelectDate;
...
import com.cn.carigps.widgets.Popup;
import com.cn.carigps.widgets.PopupUtils;
...
public class AllLocationBaidu extends Activity {
    ...
    private MyApplication glob;//全局变量
...
//                            mBaiDuMap.animateMapStatus(MapStatusUpdateFactory.newLatLng(end_point));
                ...
                if (l != null) {
                    ...
                    Marker marker = (Marker) mBaiDuMap
                            .addOverlay(new MarkerOptions().position(point)
                                    .icon(BitmapDescriptorFactory.fromResource(R.drawable.x_vehicle0))
                                    .anchor(0.5f, 0.5f));
                    setMarkerIcon(l, marker);
    ...
    public void setMarkerIcon(Location location, Marker marker) {
        try {
            ...
            */
            int resourceId = getDrawableIdWithAngle("x_", location.getAngle());
            if (location.getVehStatus() == 1) {
                marker.setTitle("online");
//                marker.setIcon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("", location.getAngle())));
                resourceId = getDrawableIdWithAngle("x_", location.getAngle());
            }
...
//                marker.setIcon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("d_", location.getAngle())));
    ...
    public InfoWindow getInfoWindow(LatLng point) {
        if (mInfoWindow != null)
            mBaiDuMap.hideInfoWindow();
        return new InfoWindow(infoWindowView, point, -50);
    }
