...
import com.cn.carigps.widgets.PopupDialog;
            ...
            for (int j = 0; j < glob.vehicleListData.get(i).size(); j++) {
                ...
                if (l != null) {
                    ...
                    Marker marker = mMap.addMarker(new MarkerOptions()
                            .icon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("x_", 0))).position(point)
                            .title(v.getVehNoF()).snippet("")
                            .anchor(0.5f, 0.5f).infoWindowAnchor(0f, 0f));

                    try {
                        int resourceId = getDrawableIdWithAngle("x_", 0);
                        if (l.getVehStatus() == 1) {
                            marker.setTitle("online");
//                            marker.setIcon(BitmapDescriptorFactory.fromResource(getDrawableIdWithAngle("", l.getAngle())));
                            resourceId = getDrawableIdWithAngle("x_", l.getAngle());
                        }
                        if (l.getVehStatus() == 2) {
