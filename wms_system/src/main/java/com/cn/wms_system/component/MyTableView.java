...
//	private int selectedBackgroundColor = Color.argb(200, 224, 243, 250);
	 ...
	 *            内容列表
	 */
	public MyTableView(Context context, String[] title, List<String[]> data, boolean canSelected) {
		super(context);
...
//			}
	...
	class CAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			LayoutItem item;
			if (convertView == null) {
				convertView = View.inflate(context, R.layout.item_tableview, null);
				item = new LayoutItem();
				item.itemContent = (LinearLayout) convertView.findViewById(R.id.item_content);
//				item.itemOperate = (Button) convertView.findViewById(R.id.item_operate);
				convertView.setTag(item);
			} else {
				item = (LayoutItem) convertView.getTag();
				item.itemContent.removeAllViews();
//				item.itemOperate.setVisibility(View.GONE);
			}
			...
			if (getSelectedPosition() == position) { // 为当前选中行
				item.itemContent.setBackgroundColor(selectedBackgroundColor);
			} else if (rowsBackgroundColor != null
					&& rowsBackgroundColor.length > 0) {
				item.itemContent.setBackgroundColor(rowsBackgroundColor[position
			...
			for (int i = 0; i < title.length; i++) {
				if (title[i].compareTo("操作") == 0 && isCanSelected()) {
					...
					button.setLayoutParams(Constants.VERTICAL_FILL_LAYOUTPARAMS);
					...
					if (i < dataItem.length) {
						textView.setText(dataItem[i]);
					}
					item.itemContent.addView(textView);
				}
			}
