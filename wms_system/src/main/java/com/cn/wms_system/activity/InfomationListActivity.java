...
public class InfomationListActivity extends Activity {
	...
	private OnItemClickListener onItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (bundle.getInt("selected_fun_index", 1) == 5) {
				// 点击报表无响应
				/*
				 * if (bundle.getInt("selected_child_fun_item", 0) == 0 ||
				 * bundle.getInt("selected_child_fun_item", 0) == 6 ||
				 * bundle.getInt("selected_child_fun_item", 0) == 7 ||
				 * bundle.getInt("selected_child_fun_item", 0) == 8)
				 */
				return;
			}
	...
	private Handler myHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case Constants.GET_PLAN_LIST_MESSAGE:
				data.clear();
				data.addAll(progressData(Constants.replaceEnter(webOperate
						.getResult())));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (bundle.getInt("selected_fun_index", 1) <= 3) {
			...
			tableView = new TableView(getApplicationContext(), titleVale, data,
					true);
		}
		/** 库房作业管理 */
		if (bundle.getInt("selected_fun_index", 1) == 4) {
		...
		// horizontalScrollView.addView(tableView,
		// Constants.FILL_FILL_LAYOUTPARAMS);
					...
					case 0:
						...
						if (requiredDate) {
							params.put(
									...
									"{1;"
											+ bundle.getInt(
													"selected_fun_index", 1)
											+ ";一工厂;"
											+ ((bundle.getInt(
													"selected_fun_index", 1) == 1) ? (preferences
													.getString("username", "")
													.split(",")[0] + ";") : "")
											+ contentHolder.editText1
													.getEditableText()
						...
						params.put(
								"s",
								"{1;"
										+ bundle.getInt("selected_fun_index", 1)
										+ ";一工厂;"
										+ ((bundle.getInt("selected_fun_index",
												1) == 1) ? (preferences
												.getString("username", "")
												.split(",")[0] + ";") : "")
										+ "}");
				...
				if (bundle.getInt("selected_fun_index", 1) == 4) {
					params.put(
							...
							"{"
									...
									+ ";}");
					// //发送获取送检出库、送检返还、返修入库、返修出库、部品退库请求
					// if (bundle.getInt("selected_child_fun_item", 1) >= 1
					// && bundle.getInt("selected_child_fun_item", 1) <= 5)
					// params.put("s", "{" + (14 +
					// (bundle.getInt("selected_child_fun_item", 1) - 1) * 3) +
					// ";}");
				}
